...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        self.conv1_alpha = nn.Conv2d(in_channels=1, out_channels=width // 2, kernel_size=3, stride=2, padding=1,
                                     bias=False)
...
class CustomResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None,
                 design_details=None,
                 text_layer=False, i=0,
                 lora_adapt=False, rank=16,
                 ):
        super().__init__()
    ...
    def forward(self, inputs, return_attn=False):
        ...
        x = x + self.mlp(self.ln_2(x))
        if return_attn:
            return [x, compound_prompts_deeper, counter], attn
        else:
...
class CustomTransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, lora_adapt=False, rank=16,
        ...
        if current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(
                *[CustomResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
            self.resblocks = nn.Sequential(
                *[CustomResidualAttentionBlock(width, heads, attn_mask, lora_adapt=lora_adapt, rank=rank) for _ in
                  range(layers)])
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 lora_adapt=False, rank=16, design_details=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        self.conv1_alpha = nn.Conv2d(in_channels=1, out_channels=width, kernel_size=patch_size, stride=patch_size,
                                     bias=False)
        ...
        self.transformer = CustomTransformer(width, layers, heads, lora_adapt=lora_adapt, rank=rank, design_details=design_details)

        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor, alpha=None, shared_ctx=None, compound_deeper_prompts=None, return_attn=False):
        ...
        if return_attn:
            x, attn_last = self.transformer([x, compound_deeper_prompts, 0], return_attn=True)
        else:
            x = self.transformer([x, compound_deeper_prompts, 0], return_attn=False)
        x = x[0]
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        # if self.adapter_image is not None:
        #     nn.init.normal_(self.adapter_image.fc[0].weight, std=0.02)
        #     nn.init.normal_(self.adapter_image.fc[0].weight, std=0.02)
        # if self.adapter_text is not None:
        #     nn.init.normal_(self.adapter_text.fc[0].weight, std=0.02)
        #     nn.init.normal_(self.adapter_text.fc[0].weight, std=0.02)
...
def build_model(state_dict: dict, lora_adapt=False, rank=16, design_details=None):
    ...
    model = CLIP(
        ...
        lora_adapt=lora_adapt, rank=rank, design_details=design_details
    )
