...
class MambaInnerFnNoOutProj2(torch.autograd.Function):

    @staticmethod
    @custom_fwd
    def forward(ctx, x, z, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
                C_proj_bias=None, delta_softplus=True, checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
        ...
        if checkpoint_lvl >= 1:  # Will recompute conv1d_out and delta in the backward pass
            conv1d_out, delta = None, None
        ctx.save_for_backward(x, z, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              delta_proj_weight, conv1d_out, delta,
                              A, B, C, D, delta_bias, scan_intermediates, out)
        # return rearrange(out_z, "b d l -> b l d")
        return out_z
    ...
    def backward(ctx, dout):
        # dout: (batch, seqlen, dim)
        (x, z, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight, delta_proj_weight, 
         conv1d_out, delta, A, B, C, D, delta_bias, scan_intermediates, out) = ctx.saved_tensors
        L = x.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        ...
        dconv1d_bias = dconv1d_bias if conv1d_bias is not None else None
        dconv1d_weight = rearrange(dconv1d_weight, "d w -> d 1 w")
        return (dx, dz, dconv1d_weight, dconv1d_bias, dx_proj_weight, ddelta_proj_weight,
                dA, dB, dC, dD,
                ddelta_bias if delta_bias is not None else None,
                dB_proj_bias, dC_proj_bias, None)

class MambaDetailInnerFnNoOutProj(torch.autograd.Function):

    @staticmethod
    @custom_fwd
    def forward(ctx, xz, conv1d_weights,
                conv1d_biases,
        ...
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        if torch.is_autocast_enabled():     # non-execution
            x_proj_weight = x_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())
            delta_proj_weight = delta_proj_weight.to(dtype=torch.get_autocast_gpu_dtype())
        if xz.stride(-1) != 1:
            xz = xz.contiguous()
        conv1d_weights = torch.stack([rearrange(conv1d_weight, "d 1 w -> d w") for conv1d_weight in conv1d_weights])
        x, z = xz.chunk(2, dim=1)   # import step: divide x z
        conv1d_biases = torch.stack([conv1d_bias.contiguous() if conv1d_bias is not None else None for conv1d_bias in conv1d_biases])
        # conv1d forward
        # conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias,None, True)
        conv1d_out = conv1d_detail_forward(x, conv1d_weights, conv1d_biases, True)
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, 'b d l -> (b l) d'), x_proj_weight)  # (bl d)
        ...
        if checkpoint_lvl >= 1:  # Will recompute conv1d_out and delta in the backward pass
            conv1d_out, delta = None, None
        ctx.save_for_backward(xz, conv1d_weights, conv1d_biases, x_dbl, x_proj_weight,
                              delta_proj_weight, conv1d_out, delta,
                              A, B, C, D, delta_bias, scan_intermediates, out)
        # return rearrange(out_z, "b d l -> b l d")
        return out_z
    ...
    def backward(ctx, dout):
        ...
        (xz, conv1d_weights, conv1d_biases, x_dbl, x_proj_weight, delta_proj_weight, 
         conv1d_out, delta, A, B, C, D, delta_bias, scan_intermediates, out) = ctx.saved_tensors
        ...
        dx, dconv1d_weights, dconv1d_biases = conv1d_detail_backward(x, conv1d_weights, conv1d_biases, dconv1d_out, dx)
        
        dconv1d_biases = torch.stack([dconv1d_bias if conv1d_biases is not None else None for dconv1d_bias in dconv1d_biases])
        dconv1d_weights = torch.stack([rearrange(dconv1d_weight, "d w -> d 1 w") for dconv1d_weight in dconv1d_weights])
        return (dxz, dconv1d_weights, dconv1d_biases, dx_proj_weight, ddelta_proj_weight,
                dA, dB, dC, dD,
                ddelta_bias if delta_bias is not None else None,
                dB_proj_bias, dC_proj_bias, None)
...
class MambaInnerFn(torch.autograd.Function):
    ...
    @custom_fwd
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                out_proj_weight, out_proj_bias,A, B=None,C=None,
                D=None, delta_bias=None, B_proj_bias=None,
                C_proj_bias=None, delta_softplus=True,checkpoint_lvl=1):
        """
             xz: (batch, dim, seqlen)
        """
        ...
        #add out_z,12.25 #h:(batch,768,16)
...
def mamba_inner_fn_no_out_proj2(
    x, z, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True
):
    return MambaInnerFnNoOutProj2.apply(x, z, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
