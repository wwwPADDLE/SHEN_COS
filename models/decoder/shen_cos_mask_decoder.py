...
from .shen_cos_decoder_common import LayerNorm2d
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        masks, edge, iou_pred, dbg = self.predict_masks(
            image_embeddings=image_embeddings,
            edge_embeddings=edge_features,
            image_pe=image_pe,
            sparse_prompt_embeddings=sparse_prompt_embeddings,
            dense_prompt_embeddings=dense_prompt_embeddings,
            return_debug = self.debug,
        )
        ...
        # Prepare output
        if self.debug:
            return masks, edge, iou_pred, dbg
        else:
    ...
    def predict_masks(
        ...
        # Run the transformer
        hs, src_out = self.transformer(src, pos_src, tokens, cond_embedding)
        iou_token_out = hs[:, 0, :]
        mask_tokens_out = hs[:, 1 : (1 + self.num_mask_tokens), :]

        # Upscale mask embeddings and predict masks using the mask tokens
        src_feat = src_out.transpose(1, 2).view(b, c, h, w)      # (B,C,Hs,Ws)
        upscaled_embedding = self.output_upscaling(src_feat)	# (B,C2,Hm,Wm)
        ...
        B, C2, Hm, Wm = upscaled_embedding.shape
        masks_pre   = (hyper_in[:, :self.num_mask_tokens-1] @ upscaled_embedding.view(B, C2, Hm * Wm)).view(B, -1, Hm, Wm)
        edge_logits = (hyper_in[:, self.num_mask_tokens-1:] @ edge_embedding.view(B, C2, Hm * Wm)).view(B, -1, Hm, Wm)
        edge = self.sigmoid(edge_logits)
        masks = masks_pre * edge + masks_pre
