...
def calc_ovcamo(pre: np.ndarray, gt: np.ndarray, pre_cls: str, gt_cls: str, gt_path: str = None):
    ...
    metric_objs = {n: METRIC_MAPPING[n]() for n in metric_names}
    for n, obj in metric_objs.items():
        obj.step(pre, gt, pre_cls, gt_cls)

    numerical_results = defaultdict(dict)
    for m_name, m_obj in metric_objs.items():
